...
class ModelExtensionPaymentCOD extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('payment_cod_total') > 0 && (int) $this->config->get('payment_cod_total') > (int) $total) {
			$status = false;
		} elseif (!$this->cart->hasShipping() && (!isset($address['forceDisplay']) || !$address['forceDisplay'])) {
			$status = false;
