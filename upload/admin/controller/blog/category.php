...
class ControllerBlogCategory extends Controller {
  ...
  public function getList() {
    ...
    $data = [
      'categories'    => [],
      'add'           => $this->url->link('blog/category/add', 'user_token=' . $user_token, true),
      'delete'        => $this->url->link('blog/category/delete', 'user_token=' . $user_token, true),
  ...
  public function getForm() : void {
    ...
    $data       = [];
    $id         = $this->request->get['category_id'] ?? null;
    $user_token = $this->session->data['user_token'];
    $url        = '&' . http_build_query(array_intersect_key($this->request->get, array_flip(['sort', 'order', 'page'])));

    $data = [
      'description'   => $this->model_blog_category->getCategoryDescriptions($id),
      'column_left'   => $this->load->controller('common/column_left'),
      'footer'        => $this->load->controller('common/footer'),
      'header'        => $this->load->controller('common/header'),
  ...
  public function getPaginamtion() {
    $this->load->model('blog/category');
    $total = $this->model_blog_category->getCategoryTotal();
